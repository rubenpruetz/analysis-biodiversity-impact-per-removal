...
path_globiom = Path('/Users/rpruetz/Documents/phd/primary/analyses/cdr_biodiversity/globiom_maps1/')
...
lookup_cdr_globiom_df = pd.read_csv(path_globiom / 'lookup_table_globiom1_nc_files.csv')
...
for index, row in lookup_cdr_globiom_df.iterrows():  # use lookup to resample iiasa files
    ...
    tiff_resampler(path_globiom / file_name,
                   target_res,
                   'sum',  # sum instead of nearest to retain numerical property
                   path_globiom / file_name)
...
for index, row in lookup_cdr_globiom_df.iterrows():
    ...
    unconverted_file = rioxarray.open_rasterio(
        path_globiom / processing_file, masked=True)

    converted_file = unconverted_file * 1000 / 100  # x1000ha and ha to km2

    converted_file.rio.to_raster(path_globiom / processing_file,
                                driver='GTiff')
...
for index, row in lookup_aim_nc_df.iterrows():  # use lookup to resample uea files
    ...
    nc_file = rioxarray.open_rasterio(path_aim / input_file,
                                      decode_times=False,
                                      band_as_variable=True)
    data_array_proj = nc_file.rio.write_crs('EPSG:4326')
    data_array_proj.rio.to_raster(
        path_aim / 'temp_large_file.tif', driver='GTiff')

    with rs.open(path_aim / 'temp_large_file.tif') as src:
        data = src.read(band)
        profile = src.profile.copy()
        profile.update(count=1)
    with rs.open(path_aim / output_name, 'w', **profile) as dst:
        dst.write(data, 1)
...
for scenario in scenarios:
    for year in years:
        ...
        unmanaged_forest = rioxarray.open_rasterio(path_aim / unmanaged_forest,
                                                   masked=True)
        managed_forest = rioxarray.open_rasterio(path_aim / managed_forest,
                                                 masked=True)
        total_forest = unmanaged_forest + managed_forest

        total_forest.rio.to_raster(path_aim / output_name,
                                   driver='GTiff')
...
for scenario in scenarios:
    ...
    for year in range(2020, 2101, 10):
        ...
        forest_2020 = rioxarray.open_rasterio(
            path_aim / file_2020, masked=True)
        forest_yr = rioxarray.open_rasterio(
            path_aim / forest_file, masked=True)
        ...
        gain_yr.rio.to_raster(path_aim / ar_file_yr,
                              driver='GTiff')
...
for land_info in land_infos:
    for scenario in scenarios:
        for year in years:
            ...
            tiff_resampler(path_aim / processing_in,
                           target_res,
                           'nearest',  # nearest as AIM is more coarse than biodiv data
                           path_aim / processing_out)

# calculate grid area based on arbitrarily chosen input file
arbit_input = rioxarray.open_rasterio(
    path_aim / '_AIM_Afforestation_SSP1-19_2050.tif', masked=True)

bin_land = arbit_input.where(arbit_input.isnull(), 1)  # all=1 if not nodata
bin_land.rio.to_raster(path_aim / 'bin_land.tif',
                       driver='GTiff')

land_area_calculation(path_aim, 'bin_land.tif', 'AIM_max_land_area_km2.tif')
    ...
    for scenario in scenarios:
        for year in years:
            ...
            land_fraction = rioxarray.open_rasterio(
                path_aim / processing_in, masked=True)

            max_land_area = rioxarray.open_rasterio(
                path_aim / 'AIM_max_land_area_km2.tif', masked=True)

            land_area = land_fraction * max_land_area

            land_area.rio.to_raster(path_aim / processing_out,
                                    driver='GTiff')
